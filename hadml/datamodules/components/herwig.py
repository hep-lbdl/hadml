...
class HerwigEventMultiHadronDataset(InMemoryDataset):
    def __init__(self,
        ...
        num_used_hadron_types: Optional[int] = None,    # Number of used hadron types (PIDs).
        cluster_rest_frame=True,                        # Hadron kinematics boosted in the cluster 
                                                        # rest frame when True.                             
        debug=False,                                    # Number of events being processed is 
                                                        # printed when True.
        prepare_distribution_plots=False                # Preparing diagrams depicting processed 
                                                        # data distributions. 
    ):
        ...
        self.raw_file_list = []
        for pattern in raw_file_list:
            self.raw_file_list += glob.glob(os.path.join(data_dir, "raw", pattern))
        self.processed_file_name = processed_file_name
        ...
        # Particle type map
        if self.data_dir:
            self.pids_map_path = os.path.join(self.data_dir, pid_map_fname)
    ...
    def process(self, debug=False):
        """ Processes the dataset to the self.processed_dir folder by reading them line by line 
        and calling the parsing method (self._create_data). """
    ...
    def _create_dict(self):
        """ Create a new PID-to-MostCommonID map/dictionary and call the parsing method 
        (self._create_data) to get all the hadron PIDs. """
        all_data = []

        for raw_path in self.raw_file_list:
            with open(raw_path) as f:
                data_list = [self._create_data(line, create_dict=True, n_line=i) 
                             for i, line in enumerate(f) if line.strip()]
            print()
    ...
    def _plot_dist(self, data, xlabels, ylabels=None, legend_labels=None, labels=None):
        ...
        plt.tight_layout()
        filepath = os.path.join(self.data_dir, os.path.normpath("processed/distribution_plots.pdf"))
        plt.savefig(filepath)
        print(f"Distribution diagrams have been saved in {filepath}")


    def _create_data(self, line, cluster_rest_frame=None, create_dict=False, n_line=None):
        """ Parse data presented as an event prepared in a specific format.
        May also be used to parse and return only hadron PIDs needed to create a dictionary. """
        ...
        if create_dict:
            ...
            for cluster in clusters:
                hadrons.append(
                    cluster[cluster[2] == '[ ]'].to_numpy()[:, [1,3,4,5,6]].astype(float)
                )
            hadrons = np.concatenate(hadrons)
            return hadrons[:, [0]].astype(np.int16).flatten()
 
        # Parse data in a normal way creating an initial feature set
